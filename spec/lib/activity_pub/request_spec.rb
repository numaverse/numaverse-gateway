...
describe ActivityPub::Request do
  ...
  describe 'Signing requests' do
    let(:account) { create(:confirmed_account, address: address) }
    ...
    let(:address) { "0x69259eef222c232a3cb3599a3556cc371f014121" }
    let(:federated_account) do 
      acct = account.reload.federated_account
      acct.update(private_key: private_key, public_key: public_key, local_account: account)
      acct
    end
